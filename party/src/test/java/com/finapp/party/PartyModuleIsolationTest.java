...
/**
 * The structural guarantees the {@code party} module exists to provide (P1-TSK-003).
 *
 * <p>These are assertions about the <strong>build</strong>, not about behaviour. They fail when
 * somebody adds a dependency to {@code party/build.gradle.kts} — which is the moment the
 ...
 */
@Tag("architecture")
@DisplayName("party module isolation (P1-TSK-003)")
class PartyModuleIsolationTest {
    ...
    void seesNoSiblingAndNoCompositionRoot() {
        ...
        for (String forbidden : List.of("identity", "kyc", "consent", "ledger", "accounts", "transfers", "payments", "paymentmethods", "checkout", "merchant", "app")) {
    ...
    @Test
    @DisplayName("party does depend on platform, so the guard above is not vacuous")
    void dependsOnPlatform() {
        ...
        // documented direction: party -> platform -> sharedkernel.
        for (String required : List.of("platform", "sharedkernel")) {
            assertThat(classpathEntries())
                    .as("party must depend on %s", required)
     ...
     * unrelated directory such as {@code identity-backup}.
     */
