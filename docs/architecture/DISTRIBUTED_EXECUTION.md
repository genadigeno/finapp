...
| `PaymentCreation` / `PaymentConfirmation` / `PaymentCancellation` / `PaymentCapture` (`P5-TSK-009`/`-010`) | none — all state in the rows | **Dispatch-before-call, arbitrated entirely by the database** (ADR-0046). Create: the idempotency claim's unique constraint, the actor in the fingerprint. Confirm: ten instances race the intent's conditional `REQUIRES_CONFIRMATION → PROCESSING` — one row count wins, births the one attempt (the partial index as backstop) and dispatches; the losers converge on the committed states **without calling the provider**, so a retry can never cause a second provider operation (`INV-PAY-04`: the reference is stored before anything is sent). The provider call runs **between** the two transactions holding no connection (the `P1-TSK-026` discipline, asserted at the wire by the database suite); a crash mid-call strands `AUTH_DISPATCHED` visibly for the sweeper (`P5-TSK-014`). Outcomes are conditional transitions — a synchronous response, a webhook and a sweeper resolving one operation are the same harmless race. Cancel: the same conditional edge, the loser told the truth. Capture (`P5-TSK-010`): the same shape with the money's own third wall — the posting's idempotency claim `payment-capture:<attemptId>` makes a duplicate outcome structurally unable to post twice, and the `CAPTURED` transition, the posting and the intent's `SUCCEEDED` are **one transaction**, so a rollback takes all three (proven by the injected-failure probe). The surface (`P5-TSK-011`) chains the capture after a synchronous `AUTHORIZED` and adds **no new arbitration**: N instances chaining the same attempt (an original racing a client retry) are the counted ten-way race above - `dispatchCapture`'s row count admits one dispatcher, the rest converge; a crash between the confirm's outcome and the chain strands `AUTHORIZED` visibly, resolved by any client retry (converge, then finish the capture) or by `P5-TSK-013`'s webhooks. Refund (`P5-TSK-015`, ADR-0048 §4): the same dispatch-before-call shape, with the bound at **two ranks** — the domain's lock-then-look (`FOR UPDATE` on the attempt row, then `sumNonFailedFor`, in the pinned **attempt → account lock order** so the hold's account lock always follows the attempt's) and `V004`'s advisory-locked trigger beneath it for writers that bypass our code — and the money reversed as **hold-then-post**: the dispatch places a hold (the customer's funds unspendable while the answer is unknown, `INV-BAL-04`), completion releases-and-posts in one transaction keyed `payment-refund:<refundId>`, failure releases with nothing posted, and ambiguity leaves the hold **standing**. N instances refunding one capture race at the dispatch: each either fits under the bound (its refund row and hold land) or is refused; the outcome side converges by the refund's own conditional transitions, exactly as every other resolver. The refund is also the platform's first **two-transaction keyed command** (`P5-TSK-016`): Tx1 commits the dispatch beside the claim held `IN_PROGRESS` (the executor's `begin`), Tx2 completes the claim **with the judged response** in the outcome's own transaction — so a replay renders the response of record byte-for-byte (platform `V003`'s freeze, `INV-LIFE-04`), never a re-read. The crash between the transactions is the lease's own case: the retry takes the claim over and the dispatch **converges by its `V008` dispatch key** — finds the committed row, places no second hold, re-drives the wire with the reference already stored (`INV-PAY-04`) — proven by the reconstructed-crash test. Refund webhooks resolve through the same door row below (attribution by our minted `rfd-` reference over both `INV-PAY-04` columns, ten-way race counted to one entry/one fact); an `UNKNOWN` refund with no webhook is a **standing hold** — visibly parked money — until the recorded refund-sweep deferral lands | Delegates to the rows |
| `PaymentWebhookService` (`P5-TSK-012`) | none — all state in the rows | **The inbound door, arbitrated by the platform inbox's primary key** (ADR-0047 §3, the `P0-TSK-021` mechanism): N instances receiving the same delivery race one `(consumer, dedupe_key)` insert — one runs the handler seam, a concurrent racer is told `CONTENDED` and answers 409 **unacknowledged**, so the provider redelivers and the dedupe absorbs it; a redelivery after commit is `SKIPPED_DUPLICATE`, acknowledged with its evidence still appended (every authenticated delivery retains its bytes in the dedupe record's own transaction — evidence-first, `INV-HIST-02`). Authentication is stateless per instance: HMAC over `timestamp + "." + raw bytes` with a two-sided freshness window, server-clock judged (the ADR-0014 discipline; the window is minutes, NTP-bounded skew is noise against it). **The handler carries the conditional machine transitions** (`P5-TSK-013`, ADR-0047 §4) through the one shared `PaymentOutcomes` component — the same code path as the synchronous Tx2s and the sweeper, from the attempt's own current source state, so ordering is nobody's promise: out-of-order, duplicate-with-fresh-id and racing resolvers all land on one row count, proven by the ten-way webhook race (one entry, one transition, ten statements retained). **Lock-order rule, recorded**: the outcome's key-changing `UPDATE` runs BEFORE the evidence `INSERT` in every resolver's transaction — the insert's FK takes `FOR KEY SHARE` on the attempt row, and taking it first deadlocks two concurrent resolvers (40P01, found by the race suite); evidence, dedupe and effect still commit together | Delegates to the inbox and `V005` |
| `payments` schema (`V002`–`V005`, `P5-TSK-008`) | durable | **The phase's cross-instance arbiters, all in the database, none in Java.** Duplicate attempt creation: the partial unique one-live-attempt-per-intent index (ten instances confirm, one row lands, the losers converge). Concurrent partial refunds: the sum bound judged **inside `V004`'s `BEFORE INSERT` trigger under `pg_advisory_xact_lock(3, hashtext(attempt_id))`** — namespace 3, registered below — because two INSERTs cannot see each other's uncommitted siblings and the phase's privilege design leaves no row to lock that every writer must pass (`INV-PAY-05`). Outcome races (sync vs webhook vs sweeper): every status write is bounded by the generated edge triggers, so exactly one resolver's conditional write lands whatever role wrote it; payload columns move only `NULL → value`, so a fact cannot be re-decided inside a legal edge. Evidence: append-only by trigger for every writer, the migrator included (`INV-HIST-02`) | **Yes** — the payment domain's rows |
...
| `paymentmethods.payment_method` | durable | The `transfers.beneficiary` protocols verbatim, no new primitive (`P5-TSK-004`): a **partial unique one-live index** per (party, token) over the non-terminal states arbitrates ten concurrent attaches, nine **converged onto the winner's row id** behind a savepoint — one row counted in the table; detachment is a **conditional `UPDATE`** carrying `party_id = ?` in the statement (ADR-0031), its row count converging retries and folding a stranger's attempt and already-detached into one indistinguishable `false`; `V002`'s every-writer trigger makes `DETACHED` terminal and freezes identity, token and display metadata for raw SQL and the migrator alike, so updated metadata is detach-and-reattach through the freed slot, never an edit. **Ownership is the Party's** — an instrument outlives any one commercial relationship. *(Row added by the task itself: the register-decay class's fifth occurrence was exactly this table's sibling never getting one until the exit review's hand-diff.)* | No — an instrument reference; the money is the ledger's |
| `transfers.beneficiary` | durable | A **partial unique one-live index** per (party, destination) over the non-terminal states arbitrates ten concurrent creates, with nine **converged onto the winner's row id** behind a savepoint — one row counted in the table, never inferred from return values (`P4-TSK-006`). Removal is a **conditional `UPDATE`** carrying `party_id = ?` in the statement (ADR-0031), so its row count converges retries and folds a stranger's attempt and already-removed into one indistinguishable `false`; `V003`'s every-writer trigger makes `REMOVED` terminal and freezes the identity and display name for raw SQL and the migrator alike, so a renamed destination is remove-and-recreate through the freed slot and never an edit. **Ownership is the Party's, not the Customer's** — a saved destination outlives any one commercial relationship — and the destination is validated for **existence only**, deliberately: postability goes stale by design, so each transfer's own execution re-judges it and a removal racing a transfer is the accepted race of `PHASE_4_PLAN.md` §7. *(Row added by `P4-DOC-001`'s hand-diff: the table landed in `P4-TSK-006` and its row did not — the register-decay class's fifth occurrence, and the first inside a phase rather than at its boundary.)* | No — a convenience address book; the money is the ledger's |
| `transfers.transfer` / `TransferExecution` + `TransferReversal` | durable | The row is **insert-carries-outcome** (`V002`'s grants make a `FAILED` outcome unreachable by `UPDATE`), so the execution has no lost update to have and takes its contention on **both participants' ledger-account rows, locked `FOR UPDATE` in one fixed order** (ADR-0039's enumerated set; the claim's unique constraint arbitrates retries). *This row said "the source-account row" until `P4-TST-001` measured it:* the posting's foreign key takes `FOR KEY SHARE` on the destination regardless, so locking only the source left the two participants locked in **uncontrolled order** and A→B against B→A was a cycle — 783 deadlocks against 203 domain outcomes under ten instances moving both ways. A global total order over the locked rows (the idiom `lockOwnedForUpdate` already names) makes a cycle impossible; the order is Java's `UUID` order and deliberately **not** PostgreSQL's byte order, because what a deadlock-free protocol needs is that every *instance* agree, not that the order match any particular sort (`P3-TSK-008`'s recorded disagreement, harmless here). The reversal (`P4-TSK-009`) is **lock-then-look on the transfer row**: `SELECT … FOR UPDATE`, the machine judged from the locked read *before any ledger work* — the loser of two concurrent reversals blocks, resumes onto the winner's commit and is refused with nothing posted — with the conditional `UPDATE`'s row count as belt, `V002`'s trigger edges binding raw SQL, and `V009`'s in-trigger advisory bound as the deepest layer. *(This row was added by `P4-TSK-009` rather than left to the next transition audit — the register-decay note below, finally heeded by a task instead of repaired after it.)* | **Yes**, for the movement's judgement; the money is the ledger's |
| `TransferLimitCheck` (seam) | none by contract | **Stateless today** (`PermitAllUntilPhase13`, field-free by test). The contract states where Phase 13's authority must live: any counter or window a verdict rests on is a **durable row read — and written — through the passed unit of work**, judged under the source-account lock in the execution's own transaction (`P4-TSK-010`'s in-lock probe observes the lock held), so the judgement and the movement commit atomically (`INV-CON-03`: limits enforced non-atomically are limits that do not exist). A process-local counter would be exactly ADR-0024's forbidden shape; a cache may exist only as explicitly non-authoritative, registered here first | Delegates — a refusal commits `FAILED(LIMIT_REFUSED)` on the transfer row |
| `TransferRiskDecision` (seam) | none by contract | The sibling seam, same contract verbatim: stateless today, and Phase 13's signals/decisions anchor to durable state on the execution's unit of work, in-lock. One reserved reason per seam (`FAILED(RISK_REFUSED)`) so neither implementation can commit the other's vocabulary | Delegates — a refusal commits `FAILED(RISK_REFUSED)` on the transfer row |
...
| `PaymentMetrics` caches (`P5-TSK-017`) | per-instance cached readings behind a floor | **Non-authoritative**, the same stance one phase on: the stuck-payment gauges read attempts and refunds in an `*_UNKNOWN` state, **the scrape is the schedule** (no lease, no leader, nothing written), every instance publishes the same fleet-wide answer so a dashboard aggregates with `max()` never `sum()`, and an unreadable database publishes **NaN, never zero** — a false zero would silence the one alert `INV-LIFE-03` makes necessary. The counters beside them are per-instance and monotonic (`rate()`/`sum()`), incremented **at the door after the transaction commits** and **only when the outcome's own conditional fired**, so a ten-way race counts one judgement once | No |
| `merchant.merchant` / `MerchantOnboarding` + `MerchantAdministration` (`P6-TSK-003`) | durable | **The commercial counterparty and its books, arbitrated entirely by the database.** Onboarding: the idempotency claim's unique constraint admits one caller (scope `merchant.onboard`, the fingerprint binding the operator and the request's meaning), so ten instances on one key produce ONE merchant, ONE payable ledger account, ONE audit record and ONE event - counted in four tables, the `P4-TSK-002` shape. The payable account rides `createOrConverge` behind its savepoint (the `ledger_account_one_per_owner_purpose_currency` partial index, kind-agnostic by construction), so a partially-repeated onboarding still lands on one account. **The KYB gate is a per-decision authoritative read inside the caller's unit of work** - the `AccountHolderVerification` contract verbatim, with the ORGANISATION predicate added: nothing caches it, so a customer closed on one instance is refused by every other on its very next decision, and the answer's causes are conflated (`INV-IDN-07`'s reasoning at a new boundary). Administration: the row `FOR UPDATE` first (the `P3-TSK-014` closers idiom), the edge judged by the aggregate, the write **conditional on the from-state**, the history row in the same transaction - and a retry whose target state is already the row's converges silently, one act however many operators asked. `V002`'s every-writer trigger refuses illegal edges and frozen-column edits for raw SQL and the migrator alike, proven against the MIGRATOR deliberately (the app role's narrowed grant already refuses, so only the unbindable writer proves the trigger's claim). **No balance exists to contend for**: the payable is the ledger position (`INV-MER-02`), so every future payout and fee contends on the ledger account's own lock, not here | **No** - the counterparty's relationship; the money is the ledger's |
| `merchant.fee_schedule` / `FeeSchedules` (`P6-TSK-004`) | durable | **What the platform charges, and the one place in this phase where the natural lock is unavailable by design.** Version minting is the only contended operation: ten instances adding a version to one schedule must produce ten distinct numbers, and the row they would naturally serialize on **cannot be locked** - PostgreSQL requires the `UPDATE` privilege to take a row lock and `V004` withholds it from a table nothing may ever update, so `SELECT ... FOR UPDATE` is refused outright (SQLState 42501). The immutability and the absence of a lock are one fact stated twice, not an obstacle worked around. **The arbiter is therefore the unique index on `(fee_schedule_id, version)`, which is also the queue**: a racer inserting a number an uncommitted transaction holds waits on the index, is refused when that transaction commits, re-reads and takes the next - bounded at 16 consecutive losses so a pathological storm fails loudly rather than spinning. The refusal is taken on a **savepoint**, so a lost race costs one statement rather than the whole transaction and the audit record written beside the version survives it. **Everything else here reads immutable rows**, so there is nothing to serialize: resolution takes no lock, and a version committed concurrently with `effective_from = now` may or may not be seen - *both answers are correct*, because "now" is not one instant across ten instances, and pinning whichever answer was given is what makes the choice permanent and explainable (`INV-MER-03`). **Assignment** is the only mutable thing: the MERCHANT row `FOR UPDATE` first (that table does hold an `UPDATE` grant), then insert-or-move under it, so ten concurrent first-assignments never make the primary key refuse nine - and an assignment converging on the schedule already pointed at writes nothing at all. **No cache anywhere**, and the reason differs from the API key's: a version is immutable, so caching one WOULD be safe; it is absent because it is unnecessary, not because it is unsafe | **No** - a price is a term, not a position; the money it produces is the ledger's (`P6-TSK-005`) |
| `merchant.payment_fee_pin` / `MerchantSettlement` (`P6-TSK-005`) | durable | **ADR-0050 section 3's entry, and it contends for nothing new.** The composition runs INSIDE the capture's own transaction, on the caller's connection, AFTER the conditional transition has been won - so it runs exactly once per capture however many resolvers raced, and the four lines inherit `payment-capture:<attemptId>`'s guarantee wholesale because they are ONE entry under that key. Ten instances applying one outcome therefore post one entry, credit the payable once, charge the fee once and announce `FeeAssessed` once, counted in the tables. **The pin is written at intent creation and never again**: `payment_intent_ref` is the primary key, so a second pin is refused by the key rather than by a read-then-write, and the row is immutable at three ranks (no UPDATE grant, V004's unconditional trigger reused, a port with no method that could express a change). **Nothing is resolved at capture that could have changed since**: the version is pinned, so a schedule version created mid-flight prices nothing already dispatched (`INV-MER-03`) - which is the restatement risk question 8 carried, one level down. Three assumptions are CHECKED rather than trusted, each unreachable today and each a silent mispricing if it stopped being: the captured amount is the pinned gross, the merchant's payable exists, and the account the capture credits IS that payable. All three throw, failing the whole capture - the honest outcome, because the alternative is posting two lines where four were owed. No savepoint around the composition, deliberately: `PaymentCapture`'s recorded stance for the posting itself, inherited rather than re-decided | **Yes** - this is the entry every merchant's money flows through |
| `checkout.checkout_session` + `checkout_order` / the aggregates (`P6-TSK-006`) | durable | **The race ADR-0053 section 5 is about, arbitrated with NO NEW PRIMITIVE.** The expiry sweeper and a customer's confirmation meet on one row as two conditional transitions from `OPEN`, and exactly one wins (`INV-CON-02`); the loser's row count is `false` and it re-reads. A late capture then takes `EXPIRED -> COMPLETED_LATE`, so NEITHER OUTCOME IS A DEAD END - which is the whole of `INV-MER-06`: money that landed is never orphaned by a clock, because auto-reversing it would start a SECOND movement with its own failure modes to undo a first that succeeded. **Expiry is a state the sweeper EARNS and a clock the aggregate ALSO checks**, and those are different questions rather than two answers to one: the state makes expiry countable, audited and present in history (never a `WHERE expires_at < now()` filter pretending to be a state, ADR-0053 section 4), while the clock makes the refusal timely - without it, a sweeper one minute behind is a minute in which money lands on a dead offer. **Ten completions of one session produce ONE order**: the conditional `PAYMENT_PENDING -> COMPLETED` admits one, and `UNIQUE (session_ref)` is the second rank behind it - refused by the key rather than by a read-then-write. **One payment intent per session** is a database fact: the reference is SET ONCE at the trigger, so a retry after a failed payment re-uses it (ADR-0045 section 4's shape one level down) and a second value is impossible rather than discouraged. The offer itself is FROZEN for every writer - amount, price pin, token and deadline are what they were when the offer was made - and the order is append-only at three ranks, its own `checkout.orders_are_immutable()` rather than merchant's, because a schema that depends on another schema's function cannot be created or reasoned about on its own. **A session that dies unpaid leaves NO order row at all** | **Yes** - the completion carries the capture, and the order is the commercial fact the books are reconciled against |
| The checkout flow / `CheckoutService` + `CheckoutSessions` (`P6-TSK-007`) | durable | **The phase's first whole flow, and the concurrency it adds is all CONVERGENCE rather than new arbitration.** Creation is keyed per merchant and the claim records the SESSION ID ALONE, so ten concurrent creates produce one session and exactly one of the ten answers carries the token - show-once holds under a race, and a replay has nothing to re-show (`INV-IDN-01` winning over the byte-for-byte replay discipline, the `P6-TSK-002` decision taken again). Confirmation opens the intent under a DERIVED key (`checkout:<sessionId>`), so ten racers converge on ONE intent - **and that convergence is what made the fee pin a race for the first time**: every converged caller then arrives at `MerchantSettlement.pin` with the same decision, and the primary key admits one. The pin now CONVERGES on an identical decision behind a savepoint and REFUSES a different one, which are two halves of one rule: converging is what keeps nine of ten purchases from failing, and refusing is what keeps that convergence from becoming a silent repricing (`INV-MER-03`). **A retried confirmation of a session that already PAID converges too**, rendering the outcome rather than a `409`, because a lost response is the ordinary failure of a payment flow and the customer holds only a token - there is no read surface for them to ask. The payer is re-established against the intent first, so the convergence discloses nothing to a second holder of the token. **Nothing new is serialized**: the session's conditional edges, the posting key and `UNIQUE (session_ref)` are `P6-TSK-005`'s and `-006`'s, inherited whole and counted end to end - ten confirmations of one session produce one order, one payable movement and one `OrderPaid`. **A suspension bites at AUTHENTICATION**, because the key lookup carries the merchant's standing in the join; `checkout.NotTrading` therefore guards only the interleaving where a suspension commits between that read and the command's own | **Yes** - this is the flow every merchant's money arrives through |
| The expiry sweep / `CheckoutExpirySweeper` (`P6-TSK-008`) | durable | **ADR-0024's other half again, and the cleanest instance of it on the platform: there is no external call at all.** Every instance polls; no lease, no leader. The candidate list is bounded and deterministically ordered, each row expires in its OWN transaction (a tick that swept a hundred sessions in one would let one poisoned row roll back ninety-nine other merchants' expiries), and the write is lock-then-judge-then-conditional. Ten sweepers on one overdue offer produce ONE `EXPIRED`, one history row, one audit record and one event, counted in the tables. **The guard that actually does the work under the lock is the STATE CHECK, not the row count** - a loser re-reads `EXPIRED`, finds the edge absent and returns - which is why the row-count mutation survives here and the state-check mutation does not; the row count still binds a caller that reads without the lock, and the store's own stale-snapshot test drives that path. **Two states expire and the second is the point**: `PAYMENT_PENDING` is the only terminal escape for a session whose payment failed, because ADR-0053 gave checkout no failure state. It is protected by a GRACE, and the grace is applied **at the decision, not only at the query** - a row read as `OPEN` and confirmed before the lock is judged as the `PAYMENT_PENDING` row it has become, or a customer confirming into a tick would be counted as a provider answering late. **Expiring a payment in flight cancels nothing**: the payment continues on its own machine and the capture lands `EXPIRED -> COMPLETED_LATE` (`INV-MER-06`), which is the race this phase is named for, driven both ways | **Yes** - the clock decides whether an offer stands, and a capture lands against whichever state it finds |
| The merchant refund / `MerchantSettlement.refund` (`P6-TSK-014`) | durable | **ADR-0050 §6's mirror seam, and it contends for nothing new.** The composition runs INSIDE the refund's own transaction, on the caller's connection, AFTER the conditional `complete` has been won - so it runs exactly once per refund however many resolvers raced, and the extra lines ride under `payment-refund:<refundId>` as ONE entry, inheriting `INV-IDEM-01`'s guarantee wholesale. **The proportional arithmetic is order-independent by construction**: the returned fee is `cum(after) − cum(before)` over a cumulative allocation, so any sequence of partial refunds telescopes to `cum(total)` and a fully refunded capture returns EXACTLY the assessed fee - conservation by algebra rather than by a clamp or a running total anybody has to keep. Two concurrent refunds of one payment are serialized by `payments.V004`'s refund-bound trigger (advisory lock on the attempt, namespace 3), which is what makes `refundedBefore` a committed read rather than a write-skew snapshot. **`refundedBefore` is the COMPLETED sum, not the budget's non-failed one**: reserved is not returned, and a sibling refund in flight can still fail. Nothing is resolved at refund time that could have changed since - the version is the payment's pin, so a schedule version created between the capture and the refund prices neither (`INV-MER-03`), and `returnedFee` refuses an assessment from any other version rather than trusting the lookup. **The dispatch's hold is the refund's NET (`P6-TSK-015`, ADR-0054)**, asked of the SAME composition in the dispatch transaction under the attempt lock, then placed under the payable's `FOR UPDATE` - the one row every refund, payout and capture on that payable serializes on, so ten instances refunding ten sales of one merchant admit exactly the affordable set (counted), and a capture's in-flight posting blocks the placement until it commits, so a refund is funded only by money that has landed. **The reservation cannot be made stale in the dangerous direction**: it reads the pin, the version and the gross (immutable) and the COMPLETED sum (monotone), so a stale read only widens the range of completion orders it must cover and over-reserves, never under; and it covers every order still possible, because the returned share depends on which siblings complete first | **Yes** - it moves money out of a counterparty's position |
...
transfer's **two participants, in one fixed order**). Two deliberate *absences* are themselves the design: postings take no lock,
...
| `3` | `payments.V004`'s refund-bound trigger (`P5-TSK-008`) | `hashtext(attempt_id::text)` — taken **inside the `BEFORE INSERT` trigger**, so every writer refunding one captured attempt serializes, raw SQL included; transaction-scoped, held to commit (`INV-PAY-05`). The `V009` shape verbatim, for the same reason: the insert-vs-insert predicate cannot see a concurrent sibling under `READ COMMITTED`, and the command's lock-then-look on the attempt row (`P5-TSK-015`) binds only writers that run our code. `PaymentsMigrationTest` pins the namespace so a migration cannot drift onto a sibling's |
