...
**M1.1 — A person exists and is registered.** `P1-TSK-001` … `P1-TSK-006`; **2 of 6 complete.**
...
**None in progress.** `P1-TSK-002` completed 2026-09-04. **Next: `P1-TSK-003`** — `party` and
`identity` module skeletons.
...
The first modules other than `platform` to own a schema. The boundary must exist before the
aggregates do — `P0-TSK-002`'s lesson, one phase on — and `ProductionModules` derives its coverage
from the classpath, so every existing architecture rule protects them without being edited.
