...
**M1.2 — That person can authenticate.** `P1-TSK-007` … `P1-TSK-012`; **3 of 6 complete.**
...
**None in progress.** `P1-TSK-009` completed 2026-09-06. **M1.2 is 3 of 6.**
**Next: `P1-TSK-010`** — `POST /v1/authentications`, enumeration-safe.
...
One response shape for every failure — unknown identity, wrong credential, locked account —
returning a session. `P1-TSK-008` built the timing half and `VerificationOutcome` deliberately gives
a caller nothing to branch on; this is where that decision either holds or is quietly undone by
somebody mapping an outcome to a message.
