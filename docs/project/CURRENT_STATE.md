...
**Five mutations, all caught**: the increment lost; a locked identity skipping the derivation; a
correct password clearing the lock; a lock that never expires; the crossing audited on every attempt
rather than once. 737 hermetic tests, 269 database tests.
