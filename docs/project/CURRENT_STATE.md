...
**M1.1 — A person exists and is registered.** `P1-TSK-001` … `P1-TSK-006`; **3 of 6 complete.**
...
**None in progress.** `P1-TSK-003` completed 2026-09-04. **Next: `P1-TSK-004`** — connection-pool
...
Recorded debt and transition risk **R6**. Hikari's default is 10 connections per instance; ten
instances exhaust PostgreSQL's default `max_connections` of 100 before any connection does work,
and the failure presents as instances failing readiness for pool exhaustion rather than for
anything wrong with the database. ADR-0014 says N is never 1, so this is arithmetic owed before the
ledger — and Phase 1 is the first phase with real pool usage.
