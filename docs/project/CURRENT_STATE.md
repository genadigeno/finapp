...
**M1.2 — That person can authenticate.** `P1-TSK-007` … `P1-TSK-012`; **4 of 6 complete.**
...
**None in progress.** `P1-TSK-010` completed 2026-09-06. **M1.2 is 4 of 6.**
**Next: `P1-TSK-011`** — brute-force and credential-stuffing controls.
...
| **`POST /v1/authentications` is unthrottled, and it is a CPU and memory amplifier.** Anyone who can reach the port can force unlimited Argon2id derivations | `P1-TSK-011` builds failure counting and lockout and **depends on this task**, so the plan's own sequence is endpoint first, controls second. Building a second throttling mechanism here, before the one designed for it exists, is designing the general case from one example | **Resource exhaustion, and worse than registration's.** ADR-0032 makes each attempt cost ~46 ms and ~19 MiB *by design*, so ten concurrent attempts is ~190 MiB of transient allocation on one instance - the work factor that protects a stolen credential store is the same work factor an attacker gets to spend for free. `INV-IDN-07` still holds: every response is identical, so flooding discloses nothing. Bounded today only by the fact that nothing is deployed | `P1-TSK-011` landing | Phase 1 |
...
The endpoint now exists and is unthrottled, which makes it the platform's most CPU-costly
unauthenticated operation: ~46 ms and ~19 MiB per attempt by design (ADR-0032). Failure counting and
lockout must be **database-backed** — a process-local counter is a limit that does not exist once a
second instance starts (ADR-0024) — and a locked account's response must stay indistinguishable from
an ordinary failure, which is `INV-IDN-07` meeting a control built to *change* behaviour.
