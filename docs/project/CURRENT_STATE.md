...
**M1.2 — That person can authenticate.** `P1-TSK-007` … `P1-TSK-012`; **6 of 6 complete** — but the milestone does **not** close: its stated acceptance requires a session, which is `P1-TSK-027`.
...
**None in progress.** `P1-TSK-012` completed 2026-09-06. **M1.2 is 6 of 6 by task count and does not
close** — `PHASE_1_PLAN.md` §11 requires *"an identity authenticates and receives a session"*, which
is `P1-TSK-027`.
**Next: `P1-TSK-013`** — the session aggregate.
...
**Three mutations, all caught** — two of them only after the tests were corrected.
737 hermetic tests, 280 database tests.
...
ADR-0030: server-side, authoritative in PostgreSQL, opaque identifier, **assurance as a level rather
than an MFA boolean**, and idle **and** absolute expiry both recorded on the row so a policy change
cannot retroactively extend existing sessions. It opens M1.3 — and it is also what `P1-TSK-027`
needs before M1.2 can actually close, which is the milestone-boundary defect `P1-TSK-010` recorded
