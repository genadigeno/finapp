...
| Ten transfers **into** one destination | The destination's row is locked too — see the row below; the decision is still the source's alone, because credits need no availability answer, and the projection row's own lock serialises the increment in `PostingEffect`'s fixed order |
| **Money moving both ways between one pair** | **Added by `P4-TST-001`, which found it by measuring.** The row locks for *both* participants are taken `FOR UPDATE` in one fixed order. This table originally had no such row, and the execution locked the source alone — but the posting's foreign key takes `FOR KEY SHARE` on the destination regardless (`P3-TSK-014`'s mechanism), so A→B holding `FOR UPDATE(A)` and needing `KEY SHARE(B)` against B→A holding `FOR UPDATE(B)` and needing `KEY SHARE(A)` is a cycle: **783 deadlocks against 203 domain outcomes** under ten instances. The one-directional drain cannot reach it, because a cycle needs two directions. A global total order over the locked rows is the remedy — the idiom `LedgerAccountStore.lockOwnedForUpdate` already names for multi-account closers |
