// The `consent` module (P2-TSK-003).
//
// The lawful basis for processing, held separately from authentication and authorization —
// CLAUDE.md §Domain Distinctions forbids collapsing the three, and a module boundary is what
// makes that a structure rather than a convention. Owns the versioned consent text and the
// append-only grant/withdraw history (ADR-0037).
//
...
//   module owns the `consent` schema and nothing else may change it; a single central migration
...
// grants it applies — and `INV-CNS-02` (consent history append-only, the audit-table mechanism)
// is enforceable at DB-PRIVILEGE only because the owner is not the application.
...
flyway {
    ...
    schemas = arrayOf("consent")
    defaultSchema = "consent"
    locations = arrayOf("filesystem:src/main/resources/db/migration/consent")
...
dependencies {
    // The documented direction: consent -> platform -> sharedkernel.
